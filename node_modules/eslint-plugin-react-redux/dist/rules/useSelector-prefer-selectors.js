...
module.exports = function (context) {
  ...
  return {
    CallExpression(node) {
      ...
      var selector = node.arguments && node.arguments[0];
      if (selector && (selector.type === 'ArrowFunctionExpression' || selector.type === 'FunctionExpression')) {
        reportNoSelector(context, node);
      } else if (selector && selector.type === 'Identifier' && config.matching && !selector.name.match(new RegExp(config.matching))) {
