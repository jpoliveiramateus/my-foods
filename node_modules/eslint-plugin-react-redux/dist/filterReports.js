...
var ruleComposer = require('eslint-rule-composer');
...
function normalizeMessagePlaceholders(descriptor, messageIds) {
  var message = typeof descriptor.messageId === 'string' ? messageIds[descriptor.messageId] : descriptor.message;
  if (!descriptor.data) {
    return {
      message,
  ...
  var normalizedData = Object.create(null);
  var interpolatedMessage = message.replace(/\{\{\s*([^{}]+?)\s*\}\}/g, function (fullMatch, term) {
...
function getMessageIds(rule) {
  var meta = getRuleMeta(rule);
  return meta.messages && typeof rule.meta.messages === 'object' ? meta.messages : {};
}

function getReportNormalizer(rule) {
  var messageIds = getMessageIds(rule);

  return function normalizeReport() {
    var descriptor = normalizeMultiArgReportCall.apply(undefined, arguments);
    var interpolatedMessageAndData = normalizeMessagePlaceholders(descriptor, messageIds);
...
function removeMessageIfMessageIdPresent(reportDescriptor) {
  var newDescriptor = Object.assign({}, reportDescriptor);
...
var filterReports = function filterReports(rule, getPropNameFromReactRuleMessage, getPropNameFromReduxRuleMessage) {
  return Object.freeze({
    create(context) {
      var removeProps = [];
      return getRuleCreateFunc(rule)(Object.freeze(Object.create(context, {
        report: {
          enumerable: true,
          value() {
            var reportDescriptor = getReportNormalizer(rule).apply(undefined, arguments);
            if (reportDescriptor.message.indexOf('exclude:') > -1) {
              removeProps.push(getPropNameFromReduxRuleMessage(reportDescriptor.message));
            } else {
              var propName = getPropNameFromReactRuleMessage(reportDescriptor.message);
...
module.exports = function (rules, getPropNameFromReactRuleMessage, getPropNameFromReduxRuleMessage) {
  return filterReports(ruleComposer.joinReports(rules), getPropNameFromReactRuleMessage, getPropNameFromReduxRuleMessage);
};